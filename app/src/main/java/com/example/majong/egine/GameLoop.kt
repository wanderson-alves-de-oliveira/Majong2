...
class GameLoop(private val surfaceHolder: SurfaceHolder, private val context: Context) : Thread() {
    ...
    private fun update(deltaTime: Double) {
        ...
        while (running) {
            ...
            if (canvas != null) {
                try {
                    bloquerBT = selectedTiles.filter { it.camada < -2 }.isEmpty()
...
//////////////////////////////////////////////////////////////
                        ...
                        runBlocking {
                            launch(Dispatchers.Default) {
                                ...
                                if (time2 > 0 && selectedTiles.isNotEmpty()) {
                                    botao2.camada = 1
...
//////////////////////////////////////////////
                    ...
                    if (tiles.filter { it.camada > 0 }
                            .isEmpty() && selectedTiles.isEmpty()) {
...
private fun limparSelecionados() {
    if (selectedTiles.isNotEmpty()) {
        ...
        var tilesx: MutableList<MahjongTile> =
            tiles.filter { it.camada == 0 && it.id == s[0].id }.toMutableList()
        var tilesx1: MutableList<MahjongTile> =
            tiles.filter { it.camada == 1 && it.id == s[0].id }.toMutableList()
        var tilesx2: MutableList<MahjongTile> =
            tiles.filter { it.camada == 2 && it.id == s[0].id }.toMutableList()
...
private fun embaralha() {
    ...
    var tilesx: MutableList<MahjongTile> =
        tiles.filter { it.camada > -1 }.toMutableList()
